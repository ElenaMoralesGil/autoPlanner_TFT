...
class GeneratePlanUseCase {
            ...
            fixedOccurrences.sortedBy { it.second }.forEach { (task, occurrenceTime) ->
                ...
                when (placementResult) {
                    ...
                    is PlacementResult.Conflict -> {
                        ...
                        fixedPlacementConflicts.add(
                            ConflictItem(
                                conflictingTasks = listOfNotNull(task, conflictingTask),
                                reason = "Fixed time slot conflict",
                                conflictTime = occurrenceTime
                            )
                        )
                    }
                    ...
                    is PlacementResult.Failure -> {
                        ...
                        fixedPlacementConflicts.add(
                            ConflictItem(
                                conflictingTasks = listOf(task),
                                reason = placementResult.reason ?: "Cannot place fixed task",
                                conflictTime = occurrenceTime
                            )
                        )
                    }
                ...
                when (result) {
                    ...
                    is PlacementResult.Conflict -> {
                        ...
                        unresolvedConflicts.add(
                            ConflictItem(
                                conflictingTasks = listOfNotNull(task, conflictingTask),
                                reason = result.reason ?: "Scheduling conflict",
                                conflictTime = findConflictTimeFromTimeline(
                                    timeline,
                                    task,
                    ...
                    is PlacementResult.Failure -> {
                        ...
                        val isExpiredInScope =
                            deadline != null && deadline.isBefore(LocalDateTime.now()) && deadlineDate != null && !deadlineDate.isBefore(
                                scheduleStartDate
                        ...
                        if (!isExpiredInScope) {
                            unresolvedConflicts.add(
                                ConflictItem(
                                    conflictingTasks = listOf(task),
                                    reason = result.reason ?: "Could not find suitable time slot",
                                    conflictTime = task.startDateConf.dateTime ?: LocalDateTime.of(
                                        scheduleStartDate,
                                        input.workStartTime
                                    ) // Estimate time
                                )
    ...
    private fun findConflictTimeFromTimeline(
        timeline: Map<LocalDate, List<TimeBlock>>,
        failedTask: Task,
        conflictingTaskId: Int?,
    ): LocalDateTime? {
        conflictingTaskId
            ?: return failedTask.startDateConf.dateTime // Fallback if no conflicting task ID

        for ((_, dayBlocks) in timeline) {
            val conflictingBlock = dayBlocks.find { it.taskId == conflictingTaskId }
            if (conflictingBlock != null) {
                return conflictingBlock.start
            }
        }
        return failedTask.startDateConf.dateTime
    }
