...
fun ListSelectionDialog(
    ...
    selectedListId: Long?,
    onDismiss: () -> Unit,
    onListSelected: (TaskList?) -> Unit, // Pass null for "None"
    onCreateNewList: () -> Unit,
) {
    GeneralAlertDialog(
        title = { Text("Assign to List") },
        content = {
            Box(modifier = Modifier.heightIn(max = 300.dp)) { // Limit height
                if (isLoading) {
                    LoadingIndicator()
                } else {
                    LazyColumn {
        ...
        hideDismissButton = true // No separate dismiss needed if Cancel works
    )
}
